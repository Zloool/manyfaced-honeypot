...
class TestFullPathSocketToDatabase:
    ...
    def test_detects_valid_bear_and_saves_to_sqlite(self):
        ...
        bear_data = {
            ...
            'parsed_request': {
                'command': 'GET', 'path': '/wp-login.php',
                'version': 'HTTP/1.1', 'headers': {'Host': 'honeypot'},
            },
            'is_detected': 1, 'HIVELOGIN': '',
        }

        handler = self._run_pipeline(bear_data)

        _verify_record('bots/honeypot.sqlite', ip='10.1.2.3',
                       path='/wp-login.php', detected=1)
    ...
    def test_detects_webdav_scan_and_saves_to_sqlite(self):
        ...
        bear_data = {
            ...
            'parsed_request': {
                'command': 'PROPFIND', 'path': '/webdav/',
                'version': 'HTTP/1.1', 'headers': {'Host': 'honeypot'},
            },
            'is_detected': 1, 'HIVELOGIN': '',
            'hostname': 'localhost', 'country': 'RU', 'continent': 'EU',
            'tracert': '', 'dns_name': '',
        }

        handler = self._run_pipeline(bear_data)

        _verify_record('bots/honeypot.sqlite', ip='192.168.99.99',
                       path='/webdav/', field='request_command', value='PROPFIND')
    ...
    def test_incorrect_identifier_fails_gracefully(self):
        ...
        import sys as _sys
        old = getattr(_sys.modules['manyfaced.common.settings'], 'AUTHORISEDBEARS', None)
        _sys.modules['manyfaced.common.settings'].AUTHORISEDBEARS = dict()

        try:
            unknown_bear_data = {
                'ip': '10.0.0.1', 'raw_request': 'GET /admin\r\n\r\n',
                'timestamp': '2026-04-18 22:00:00.000000',
                'parsed_request': {'command': 'GET', 'path': '/admin'},
                'is_detected': 1, 'HIVELOGIN': '',
            }
            message = make_encrypted_message('unknown_bear', unknown_bear_data, TEST_KEY)
            ...
            if old is None:
                _sys.modules['manyfaced.common.settings'].__dict__.pop('AUTHORISEDBEARS', None)
            else:
    ...
    def test_invalid_format_raises_valueerror(self):
        ...
        from manyfaced.server.server import ServerHandler
        handler = ServerHandler(
            MagicMock(server=(0, 6669), verbose=False), MagicMock()
        )
    ...
    def test_invalid_json_raises_valueerror(self):
        ...
        handler = ServerHandler(
            MagicMock(server=(0, 6670), verbose=False), MagicMock()
        )
        with pytest.raises(ValueError):
            handler.handle_request(message)
    ...
    def test_raw_request_preserved_in_database(self):
        ...
        _verify_record('bots/honeypot.sqlite', ip='172.16.0.1',
                       path='/wp-content/debug.log', field='request_raw', value=raw)

    def test_hivelogin_field_stored(self):
        """HIVELOGIN field should be stored in the login column."""
        bear_data = {
            'ip': '10.10.10.10', 'raw_request': 'GET /\r\n\r\n',
            'timestamp': '2026-04-19 00:00:00.000000',
            'parsed_request': {'command': 'GET', 'path': '/'},
            'is_detected': 1, 'HIVELOGIN': 'testuser123',
        }
    ...
    def test_detected_field_preserved(self):
        ...
        bear_data = {
            'ip': '10.10.10.20', 'raw_request': 'GET /unknown\r\n\r\n',
            'timestamp': '2026-04-19 01:00:00.000000',
            'parsed_request': {'command': 'GET', 'path': '/unknown'},
            'is_detected': UNKNOWN_HTTP, 'HIVELOGIN': '',
        }

        handler = self._run_pipeline(bear_data)

        _verify_record('bots/honeypot.sqlite', ip='10.10.10.20',
                       path='/unknown', detected=UNKNOWN_HTTP)
...
class TestServerHandlerDirect:
    ...
    def test_process_request_starts_save_process(self):
        ...
        handler = ServerHandler(
            MagicMock(server=(0, 6676), verbose=False), MagicMock()
        )
...
class TestServerHandlerKeyLookup:
    ...
    def test_get_key_returns_none_for_unknown_bear(self):
        ...
        try:
            ...
            if old is None:
                sys.modules['manyfaced.common.settings'].__dict__.pop('AUTHORISEDBEARS', None)
            else:
...
class TestSQLiteStorageDirect:
    ...
    def test_insert_and_query_record(self):
        ...
        with SQLiteStorage(db_path) as store:
            store.insert({
                ...
                'timestamp': '2026-04-19 03:00:00.000000',
                'parsed_request': {
                    'command': 'GET', 'path': '/test',
                    'version': 'HTTP/1.1', 'headers': {'Host': 'x'},
                },
                'is_detected': 1, 'raw_request': 'GET /test\r\n\r\n',
                'ua': 'TestAgent', 'country': 'US', 'continent': 'NA',
                'tracert': '', 'dns_name': '',
            })
    ...
    def test_insert_handles_extra_fields(self):
        ...
        with SQLiteStorage(db_path) as store:
            store.insert({
                'ip': '5.6.7.8', 'timestamp': '2026-04-19 04:00:00',
                'extra_field': 'ignored',
            })
