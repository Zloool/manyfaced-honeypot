...
def test_build_payloads_drops_benign_and_favicon_and_ranks_crit_above_info():
    ...
    assert len(out) == 2
    # crit (login captured) must rank ABOVE info. out is now (raw, decoded) tuples.
    assert out[0][0].startswith('POST /login')
    # benign + favicon + bare HEAD all gone.
    joined = '\n'.join(raw for raw, _decoded in out)
...
def test_build_payload_uses_interesting_raws(tmp_path, monkeypatch):
    ...
    joined = '\n'.join(raw for raw, _decoded in payload['payloads'])
