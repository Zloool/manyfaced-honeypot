"""RabbitMQHandler - scaffold stub for issue #298.

TODO: replace the placeholder page with a realistic RabbitMQ impersonation
...
class RabbitMQHandler(HTTPHandlerBase):
    """RabbitMQ honeypot handler (scaffold)."""
    ...
    def _main_page(self) -> str:
        """RabbitMQ placeholder page (scaffold)."""
        return (
            '<!DOCTYPE html><html><head><title>RabbitMQ</title></head>'
            f'<body><h1>RabbitMQ</h1>'
            f'<p>Service: RabbitMQ 3.12.0</p>'
            '</body></html>'
        )
