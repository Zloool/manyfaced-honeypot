...
class Config:
    ...
    def generate_config_file(self, path: Path | str | None = None) -> Path:
        ...
        def _toml_str(value: str) -> str:
            ...
            # like C:\Users\... is otherwise an invalid escape (issue: config
            # unparseable on Windows, crashes Config.load at import time).
        ...
        lines = [
            ...
            f'honeyfolder = {_toml_str(self.HONEYFOLDER)}',
            '# Port listening mode: "single", "top", or "all"',
            f'port_mode = {_toml_str(self.HONEY_PORT_MODE)}',
            '# Comma-separated ports for top mode (empty = use defaults)',
            f'top_ports = {_toml_str(self.HONEY_TOP_PORTS)}',
            '',
            '[hive]',
            ...
            f'path = {_toml_str(self.DB_PATH)}',
            f'pg_host = {_toml_str(self.DB_PG_HOST)}',
