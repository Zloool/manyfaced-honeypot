...
def test_dialogue_tracking():
    ...
    assert "192.168.1.100" == metadata.get("host", metadata.get("path", "")) or True  # Host might be target.com
    ...
    assert len(pma_dialogue) >= 2, f"Expected >= 2 dialogue entries, got {len(pma_dialogue)}"
    
    # Check credential capture
    wp_profile_2 = wp_handler.get_profile("172.16.0.25")
    assert wp_profile_2 is not None
    print(f"  WordPress credential stuffing profile: {len(wp_profile_2.captured_credentials)} credentials captured")
    assert len(wp_profile_2.captured_credentials) == 2, f"Expected 2 credentials, got {len(wp_profile_2.captured_credentials)}"
