...
SPECS = [
    (287, "thinkphp", "ThinkPHPHandler", "THINKPHP_HTTP", "ThinkPHP", "8.0.0",
     ["/index.php", "/public/index.php", "/thinkphp"], "ThinkPHP"),
    (286, "laravel", "LaravelHandler", "LARAVEL_HTTP", "Laravel", "10.48.0",
     ["/laravel", "/_ignition", "/storage/logs/laravel.log", "/vendor/laravel"], "Laravel"),
    (282, "zabbix", "ZabbixHandler", "ZABBIX_HTTP", "Zabbix", "6.4.0",
     ["/zc", "/evox/about", "/zabbix.php", "/api_jsonrpc.php"], "Zabbix"),
    (278, "elasticsearch", "ElasticsearchHandler", "ELASTIC_HTTP", "Elasticsearch", "8.12.0",
     ["/_cat", "/_search", "/_cluster", "/_nodes", "/query"], "Elasticsearch"),
    (276, "gitlab", "GitLabHandler", "GITLAB_HTTP", "GitLab", "16.8.5",
     ["/sdk/weblanguage", "/users/sign_in", "/api/v4", "/explore"], "GitLab"),
    (298, "rabbitmq", "RabbitMQHandler", "RABBITMQ_HTTP", "RabbitMQ", "3.12.0",
     ["/api/overview", "/api/queues", "/api/exchanges", "/rabbitmq"], "RabbitMQ"),
    (296, "jupyter", "JupyterHandler", "JUPYTER_HTTP", "Jupyter", "4.2.0",
     ["/jupyter", "/lab", "/notebooks", "/tree"], "Jupyter"),
    (295, "plex", "PlexHandler", "PLEX_HTTP", "Plex", "1.32.0",
     ["/web/index.html", "/status/sessions", "/identity", "/plex"], "Plex"),
    (291, "grafana", "GrafanaHandler", "GRAFANA_HTTP", "Grafana", "10.4.0",
     ["/grafana", "/prometheus", "/api/datasources", "/api/health"], "Grafana"),
    (279, "solr", "SolrHandler", "SOLR_HTTP", "Apache Solr", "9.5.0",
     ["/solr/admin/info/system", "/solr/admin/cores", "/solr"], "Solr"),
    (297, "redis_admin", "RedisAdminHandler", "REDIS_ADMIN_HTTP", "Redis Admin", "2.0.0",
     ["/redis-admin", "/redis-commander", "/redis"], "Redis"),
    (293, "magento", "MagentoHandler", "MAGENTO_HTTP", "Magento", "2.4.6",
     ["/magento", "/admin", "/index.php/admin"], "Magento"),
    (289, "squid", "SquidHandler", "SQUID_HTTP", "Squid", "6.8.0",
     ["/squid", "/cachemgr", "/cgi-bin"], "Squid"),
    (288, "hnap", "HNAPHandler", "HNAP_HTTP", "HNAP", "1.0",
     ["/HNAP1", "/hnap", "/post_login.xml"], "HNAP"),
    (285, "aws_creds", "AWSHandler", "AWS_HTTP", "AWS", "1.0",
     ["/.aws/credentials", "/aws/credentials", "/.env.aws"], "AWS"),
    (281, "spring", "SpringHandler", "SPRING_HTTP", "Spring Boot", "3.2.0",
     ["/actuator", "/actuator/health", "/api/env"], "Spring"),
    (280, "atlassian", "AtlassianHandler", "ATLASSIAN_HTTP", "Confluence", "8.5.0",
     ["/wiki", "/login.action", "/setup.action", "/rest/api"], "Confluence"),
    (277, "nextjs", "NextjsHandler", "NEXTJS_HTTP", "Next.js", "14.1.0",
     ["/_next", "/__nextjs_action", "/vercel.json"], "Next.js"),
    (274, "k8s", "KubernetesHandler", "KUBERNETES_HTTP", "Kubernetes", "1.29.0",
     ["/api", "/apis", "/healthz", "/readyz", "/metrics", "/version"], "Kubernetes"),
    (294, "nginx_probe", "NginxProbeHandler", "NGINX_PROBE_HTTP", "Nginx", "1.24.0",
     ["/nginx_status", "/server-status", "/stub_status", "/status"], "nginx"),
    (284, "iot", "IoTHandler", "IOT_HTTP", "IoT Router", "1.0",
     ["/boaform/admin/formlogin", "/apply.cgi", "/cgi-bin", "/getcfg.php"], "IoT"),
    (283, "mcp", "MCPHandler", "MCP_HTTP", "MCP", "2024.11.0",
     ["/mcp", "/sse"], "MCP"),
    (275, "docker", "DockerHandler", "DOCKER_HTTP", "Docker Registry", "25.0.0",
     ["/v2/_catalog", "/v2", "/version", "/info", "/_ping"], "Docker"),
    (292, "dbadmin", "DBAdminHandler", "DBADMIN_HTTP", "Adminer", "4.8.1",
     ["/adminer", "/sqlbuddy", "/dbadmin", "/myadmin", "/adminer.php"], "Adminer"),
]
...
def routes_source(spec):
    ...
    for i, p in enumerate(paths):
        key = f'{svc}_{i}'
        route_lines.append(
            f"    Route(PathExact('{p}'), _{svc}(), {const}, '{key}'),")
        if p.count('/') >= 2 and not p.rstrip('/').endswith(('.php', '.xml', '.json', '.html', '.log', '.txt')):
            parent = p.rstrip('/')
